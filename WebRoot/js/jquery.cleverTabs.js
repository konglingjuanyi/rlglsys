...
function CleverTabs(self, options) {
    ...
    self.addClass("ui-tabs ui-widget ui-widget-content ");
    this.wrapper = self;
    var el = self.find('ol,ul').eq(0);
    this.element = el;
    this.element.addClass("ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ");
    ...
    if (this instanceof CleverTabs) {
        CleverTabs.prototype.resizePanelContainer = function () {
            if (pc.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
            	// qinqj update 20130927
                //var height = self.height() - el.height() - 30;
                var height = self.height() - el.height();
...
CleverTabs.prototype.add = function (options) {
    ...
    if (exsitsTab) {
        /* 20130826 qinqj start update */
    	if (top.topFlg != undefined) {
    		//top.isSubmitted = false;
    	}
        /* 20130826 qinqj end update */
    ...
    .replace(/#\{liclass\}/g, 'ui-state-default')
    ...
    if (self.panelContainer.attr('id').indexOf('cleverTabsPanelContainer') === 0) {
        ...
        var height = self.wrapper.height() - self.element.height();
...
//        + '</ul>'),
        element: $('<ul id="cleverTabsContextMenu">'
                + '<li id="mnuEnabl" ><a href="#enabled"><span class="ui-icon ui-icon-pencil" style="float: left; margin-right: 5px;"></span>有效</a></li>'
                + '<li id="mnuDisalb"><a href="#disable"><span class="ui-icon ui-icon-cancel" style="float: left; margin-right: 5px;"></span>无效</a></li>'
                + '<li id="mnuLock" ><a href="#lock"><span class="ui-icon ui-icon-locked" style="float: left; margin-right: 5px;"></span>加锁</a></li>'
...
CleverTabs.prototype.clear = function () {
    ...
    for (var i = self.lockOnlyOne && !hasLock ? 1 : 0; i < lis.size(); i++) {
