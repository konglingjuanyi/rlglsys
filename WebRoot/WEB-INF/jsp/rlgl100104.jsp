						...
						<div class="am-panel am-panel-secondary am-margin">
							<div class="am-panel-hd">
								<h3 class="am-panel-title">
									已选课件列表 1<input type="button" id="btn_cancel" value="取消课程"
							...
							<thead>
								...
								<s:iterator value="courseSelectedList" status="L" var="course">
									<my:trStripe index="${L.index}" delflg="${is_prep}">
										<td><s:if test="is_prep==1">
												<input type="checkbox" id="xz_ckb" disabled="disabled"
													value="${course.course_id}" name="xz_ckb">
											</s:if> <s:else>
												<input type="checkbox" id="xz_ckb"
													value="${course.course_id}" name="xz_ckb">
											</s:else> </input></td>
										<td>${L.index +1}</td>
										<td><s:if test="course_name==''">
												...
												<a
													onclick="showDiv('<s:property value='course_id' />','<s:property value='course_explain' />','<s:property value='course_image_url' />');"><s:property
														value='course_name' /></a>
											</s:else></td>
										<td><s:property value='course_expert_unit' /></td>
										<td><s:if test="course_support == 001">好医生</s:if> <s:if
												test="course_support == 002">华医网</s:if> <s:if
												test="course_support == 003">24小时在线</s:if> <s:if
												test="course_support == 004">自主制作</s:if> <s:if
												test="course_support == 005">其他</s:if></td>
										<td><s:property value='end_date' /></td>
										<td><span style="color: #00AA00"><s:property
													value='xuefen' />(分)</span></td>
										<td><s:if test="is_prep==0">
