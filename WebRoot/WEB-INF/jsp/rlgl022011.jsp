...
/* 	//页面初始化时候，给学分年度赋值
   ...
   $(document).ready(function(){
     ...
     $("#form1").attr("action", "rlgl022004Search.action");
   ...
   //申请学分按钮
	...
	function checkFlg(){
	   ...
	   if ($("#xuefeileibie").val() =="001") {
		    var i = parseFloat($("#zongxuefen").val());
		...
		<s:form name="form1" id="form1" method="post"
			...
			<div id="content">
				...
				<div style="position: relative">
					...
					<tr>
							<td >学分年度：<s:textfield id="credit_year"
									name="credit_year" maxLength="4" width="8"
									onkeyup="clearNoNum(this)" /></td>
							<td>证件编号：<s:property value='%{userId}' /></td>
						</tr>
						<tr>
							<td >课程名称：<s:textfield id="course_name"
									name="course_name" maxLength="50" width="8" /></td>
							<td  align="right"><input type="button"
								class="am-btn am-btn-success  am-btn-block"  value="查询" name="btnSearch" id="btnSearch">
								<s:hidden id="errormsg" name="errmsg" value=""></s:hidden></td>
						</tr>
					</table>
				</div>
				<br>
				...
				<s:if test="%{playFlag==1}">
						...
						<s:iterator value="courseExamsList" status="L" id="course">
							<my:trStripe index="${L.index}">
								...
								<td height="20" class="tdc"><s:property value='course_id' />
								</td>
								<td height="20" class="tdc"><s:property value='course_name' />
								</td>
								<td height="20" class="tdc"><s:property value='credit' /></td>
								<s:hidden name="credit" id="credit" />
								<td height="20" class="tdc"><s:property
										value='course_price' /></td>
								<td height="20" class="tdc"><s:property value='exams_time' />
								</td>
								<td height="20" class="tdc"><s:property value='credit_year' />
								</td>
								<td class='tdc'><s:if test="isapply==0">
